...
if(isset($_POST['actualizar'])) {
    ...
    $correoDuplicado = $mysqli->prepare("SELECT id_usuario FROM usuarios WHERE correo = ? AND id_usuario != ?");
    $correoDuplicado->bind_param("si", $nuevocorreo, $id);
    $correoDuplicado->execute();
    $correoDuplicado->store_result();

    if($correoDuplicado->num_rows > 0){
        $mensajeActualizarDatos = "El correo ya está en uso por otro usuario";
        $correoDuplicado->close();
    } else {
        $stmt = $mysqli->prepare("UPDATE usuarios SET nombre = ?, correo = ?, telefono = ?, contraseña = ? WHERE id_usuario = ?");
        $stmt->bind_param("ssssi", $nuevonombre, $nuevocorreo, $nuevoTelefono, $nuevaContraseña, $id);

        if($stmt->execute()) {
            $mensajeActualizarDatos = "Datos actualizados correctamente";
        } else {
            $mensajeActualizarDatos = "Error al actualizar los datos: ".$stmt->error;
        }
        $stmt->close();
    }
}
...
$mensaje = "";
if(isset($_POST['nombreContacto']) &&  isset($_POST['apellidosContacto']) &&  isset($_POST['correoContacto']) &&  isset($_POST['telefonoContacto']) &&  isset($_POST['mensajeContacto']))
    {
        $nombreContacto = $_POST['nombreContacto'];
        $apellidosContacto = $_POST['apellidosContacto'];
        $emailContacto = $_POST['correoContacto'];
        $telefonoContacto = $_POST['telefonoContacto'];
        $mensajeUsuario = $_POST['mensajeContacto'];
        $stmt = $mysqli->prepare("INSERT INTO contacto (nombre, apellidos, correo, telefono, mensaje) VALUES (?, ?, ?, ?, ?)");
        $stmt->bind_param("sssss",$nombreContacto, $apellidosContacto, $emailContacto, $telefonoContacto, $mensajeUsuario );
...
<body>
 ...
 <!-- NAVBAR -->
  ...
  <section class="my-5">
    ...
    <div class="card-body">
        <form  action="" name="actualizar" method="POST" class=" text-center">
            <?php if(!empty($mensajeActualizarDatos)) { ?>
             ...
             <div class="form-group row">
                    ...
                    <div class="col-sm-9">
                        <input type="text" class="form-control" id="nombre" placeholder="Escribe el nuevo nombre de usaurio" maxlength="100" name="nombre" value="<?php echo $nombre;?>" required> 
                    </div>
              </div>
             <div class="form-group row">
                    <label class="col-sm-3 col-form-label"  for="correo" > Correo: </label> 
                    <div class="col-sm-9">
                        <input type="text" class="form-control" id="correo" placeholder="Escribe el nuevo correo" maxlength="100" name="correo" value="<?php echo $email;?>" > 
                    </div>
              </div>
              <div class="form-group row">
                    <label class="col-sm-3 col-form-label"  for="telefono" > Telefono: </label> 
                    <div class="col-sm-9">
                        <input type="text" class="form-control" id="telefono" placeholder="Escribe el numero de telefono"  inputmode="numeric" pattern="[0-9]*" name="telefono" value="<?php echo $telefono;?>" required> 
                    </div>
              </div>
              <div class="form-group row">
                    <label class="col-sm-3 col-form-label"  for="contraseña" > Contraseña: </label> 
                    <div class="col-sm-9">
                        <input type="password" class="form-control" id="contraseña" placeholder="Escribe la nueva contraseña"  maxlength="100" name="contraseña" value="<?php echo $contraseña;?>" required> 
                    </div>
              </div>
               <div class="text-center form-group row p-1">
                    <div class="col-sm-12 text-center">
                    <button type="submit" name="actualizar" class="btn btn-actualizar  fw-semibold px-4"  onclick="return confirm('¿Deseas actualizar los datos de tu perfil?');">Actualizar datos</button>
                    <a href="perfil.php" class="btn btn-regresar fw-semibold px-4">Regresar</a>
                    </div>
                </div>
            </form>
    </div>
     ...
     <div class="col-md-4 mb-4">
        <div class="p-4 text-dark rounded form-box">
          ...
          <div class="row g-2 mb-2">
            <div class="col">
              <input type="text" class="form-control bg-light" placeholder="nombre" name="nombreContacto" required>
            </div>
           <div class="col">
            <input type="text" class="form-control" placeholder="apellidos" name="apellidosContacto" required>
           </div>
          </div>
          <div class="row g-2 mb-2">
            <div class="col">
              <input type="correo" class="form-control" placeholder="ejemplo@gmail.com" name="correoContacto" required>
            </div>
          <div class="col">
              <input type="text" class="form-control" placeholder="telefono" inputmode="numeric" pattern="[0-9]*" title="Solo números" name="telefonoContacto" required>
           </div>
          </div>
          <div class="mb-3">
            <textarea type="text" class="form-control" rows="3" placeholder="mensaje" name="mensajeContacto" required></textarea>
          </div>
